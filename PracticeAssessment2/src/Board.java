...
public class Board {
	// grid line width
		...
		{
			//draw the grid
			// change the color from gray to BLACK
			g.setColor(Color.BLACK);
			for (int row = 1; row < Main.ROWS; ++row) {          
				g.fillRoundRect(0, Main.CELL_SIZE * row - GRID_WIDHT_HALF,                
						Main.CANVAS_WIDTH - 1, GRID_WIDTH,                
						GRID_WIDTH, GRID_WIDTH);       
				}
			for (int col = 1; col < Main.COLS; ++col) {          
				g.fillRoundRect(Main.CELL_SIZE * col - GRID_WIDHT_HALF, 0,                
						GRID_WIDTH, Main.CANVAS_HEIGHT - 1,                
						GRID_WIDTH, GRID_WIDTH);
			}
